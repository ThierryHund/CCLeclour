...
class Utilisateurs {
	...
	public static function creer($nom, $prenom, $login, $password, $groupe, $id_mag) {
		$conn = Connection::get ();
		/*
		 * //verification du nom if(!preg_match("/^[A-Z][a-zA-Z]* [a-zA-Z]\.$/",$nom)) { throw new Exception("nom incorrect"); }else { $nom=str_replace(" "," ",trim($nom)); } //verification du prenom if(!preg_match("/^[A-Z][a-zA-Z]* [a-zA-Z]\.$/",$prenom)) { throw new Exception("nom incorrect"); }else { $nom=str_replace(" "," ",trim($nom)); } /* //verification du groupe // a adpter if($groupe!=("caisse" || "comptable" || "secours" || "administrateur")) { throw new Exception("groupe incorrect"); }
		 */
